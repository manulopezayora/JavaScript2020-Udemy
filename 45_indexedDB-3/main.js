...
TODO: Más contenido sobreIndexedDB:
- Especificación oficial -> - https://developer.mozilla.org/es/docs/Web/API/IndexedDB_API
...
- Contenido extra para buscar:
    ...
    Método onBlocked() -> Para bloquear las bases de datos en los cambios de versión.
    Objeto IDBKeyRange -> Para búsquedas en la base de datos.
    Método createIndex() -> Para la creación de índices en la abse de datos.
    Versionado de bases de datos. 
    

- Librería para IndexedDB: Dixie.js -> https://dexie.org/
...
if (indexedDB && form) {
  ...
  const deleteData = (key) => {
    ...
    //! igual que updateData(data) pero cambia el método delete(key)
    request.onsuccess = () => {
      readData();
    };
  };
